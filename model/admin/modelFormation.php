...
function recupAllFormation()
{
    ...
    return $connexion->select("SELECT * FROM article NATURAL JOIN media WHERE id_type=3");
}
...
function recupUneFormation()
{
    ...
    require_once(realpath(__DIR__ . '/../../config.php'));
    $connexion = new Connexion(NOM_BDD);
    $sqlFormation = "SELECT * FROM article NATURAL JOIN media WHERE id_article =" . $_GET['id_article'];
...
function ajoutFormation()
{
    ...
    require_once(realpath(__DIR__ . '/../../config.php'));
    $connexion = new Connexion(NOM_BDD);
    $sqlAjout = "INSERT INTO article(titre_article,contenu_article,prix_article,id_media,id_type) VALUES ( '" . $_GET['titre'] . "','" . $_GET['contenu'] . "'," . $_GET['prix'] . ",". $_GET['id_media'] .",3)";
